...
import { TRAINING_DEPARTMENT_NAV_ITEMS } from '../../config/trainingDepartmentMenuConfig';

interface TrainingDepartmentSidebarProps {
  isOpenMobile?: boolean;
  onCloseMobile?: () => void;
  onLogout?: () => void;
}

export const TrainingDepartmentSidebar: React.FC<TrainingDepartmentSidebarProps> = ({
  isOpenMobile = false,
  onCloseMobile,
  onLogout,
}) => {
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-4 py-4 border-b border-white/10 h-[73px] overflow-hidden">
          <div className="flex items-center gap-3 min-w-0 cursor-pointer" onClick={() => navigate('/training-department')}>
        ...
        <div className="flex-1 overflow-y-auto px-2.5 py-4 space-y-1.5 custom-scrollbar overflow-x-hidden">
          {TRAINING_DEPARTMENT_NAV_ITEMS.map((item) => {
            const isActive =
              location.pathname === item.path ||
              (item.id === 'dashboard' && (location.pathname === '/training-department' || location.pathname === '/training-department/dashboard'));

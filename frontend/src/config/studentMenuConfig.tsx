...
/**
 * Helper to get menu configuration by ID
 */
export const getMenuItemById = (id?: string): StudentMenuItem | undefined => {
  if (!id) return undefined;
  return STUDENT_NAV_ITEMS.find((item) => item.id === id);
};

/**
 * Helper to get menu configuration by pathname
 */
export const getMenuItemByPath = (pathname: string): StudentMenuItem | undefined => {
  const cleanPath = pathname.replace(/\/$/, '') || '/';
  return STUDENT_NAV_ITEMS.find((item) => {
    if (cleanPath === item.path) return true;
    if (item.id === 'dashboard' && (cleanPath === '/dashboard/student' || cleanPath === '/')) return true;
    if (item.id === 'academic-profile' && cleanPath === '/dashboard/academic-profile') return true;
    return false;
  });
};
